...
export const issueAndLog = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 1. Validate Entities (Must be sequential because MongoDB does not support concurrent operations on the same transaction session)
    ...
    if (!allowNegative && currentStock < payload.quantity) {
      throw new Error(`Vật tư không đủ. Khả dụng: ${currentStock}`);
    }

    // 3. Create ProductionLog first (we need its ID)
    ...
    const inventoryLog = new InventoryLogModel({
      ...
      template_key: 'issue_for_production', // Default key, could be dynamic
      template_version: 1,
      ...
      notes: payload.notes,
      status: 'approved', // Auto-approved for this workflow
      source_reference: {
        model: 'ProductionLog',
        id: productionLogId
      },
    ...
    // Also update ProductionLog with Inventory reference
    productionLog.inventory_log_id = inventoryLogId.toString();
