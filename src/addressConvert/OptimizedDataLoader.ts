...
export class OptimizedDataLoader {
  ...
  async loadFromFile(filePath?: string): Promise<void> {
    try {
      ...
      if (filePath) {
        ...
        try {
          const addressDatabase = await VietNamAddressDatabase;
          // Thư viện export trực tiếp array dữ liệu
          data = addressDatabase || (addressDatabase as any);
        } catch (importError) {
  ...
  private buildIndexes(): void {
    ...
    console.log(
      `✅ Indexes built: ${this.provinceNameIndex.size} province entries, ${this.wardNameIndex.size} ward entries, ${this.mappingIndex.size} mapping entries`
    );
  }
  ...
  findProvinceByName(name: string): Province | undefined {
    ...
    normalizedName = normalizedName.replace(/\b(tinh|thanh pho|tp|t\.p)\b/g, '').trim();

    // Tìm exact match trước
    const province = this.provinceNameIndex.get(normalizedName);
  ...
  findWardByName(name: string, provinceCode?: string): Ward | undefined {
    ...
    if (provinceCode) {
      ...
      for (const [indexedKey, indexedWards] of this.wardNameIndex.entries()) {
        if (
          indexedKey.startsWith(`${provinceCode}_`) &&
