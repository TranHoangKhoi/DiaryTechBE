...
export class VietnamAddressConverter {
  ...
  //         message: 'Địa chỉ đã đúng chuẩn, không cần chuyển đổi'
  //       };
  //     } else {
  //       result = {
  //         success: false,
  ...
  //     (this.stats.avgConversionTime * (this.stats.totalConversions - 1) + (endTime - startTime)) /
  //     this.stats.totalConversions;
  ...
  convertAddress(address: string | FullAddress): ConversionResult {
    ...
    if (relevantMappings.length > 0) {
      // ✅ Có mapping phù hợp thật sự
      const bestMapping = this.findBestMappingFast(validAddress, relevantMappings);
      ...
      if (bestMapping) {
        ...
        result = {
          ...
          originalAddress: fullAddress,
          convertedAddress,
          mappingInfo: { mappingType: 'merged' },
          message: 'Chuyển đổi thành công'
        };
      } else {
        result = {
      ...
      // ❌ Không có mapping phù hợp → kiểm tra xem là địa chỉ mới
      const provinceExists = this.findNewProvinceCached(validAddress.province);
  ...
  getWards(): Ward[] {
    return this.dataLoader.getWards();
  }
  getWardMappings(): WardMapping[] {
    return this.dataLoader.getWardMappings();
  }
